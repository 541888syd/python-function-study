...
export default function Settings() {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `english-study-backup-${new Date().toISOString().split('T')[0]}.json`;
      a.click();
  ...
  const handleReset = async () => {
    if (!confirm('确定要重置所有学习进度吗？单词本数据会保留，但练习记录和进度会被清除。此操作不可撤销！')) return;
    try {
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              每日目标数量: <strong>{settings.dailyGoal}</strong> 个
            </label>
            <input
              type="range"
              min="5"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              每日新词数量: <strong>{settings.newWordsPerDay}</strong> 个
            </label>
            <input
              type="range"
              min="1"
          ...
          <div>
            ...
            <div className="flex gap-3">
              {[
                { value: 'mixed', label: '🔀 混合随机', desc: '中→英 + 英→中' },
                { value: 'cn2en', label: '🇨🇳→🇬🇧 中译英', desc: '看中文打英文' },
                { value: 'en2cn', label: '🇬🇧→🇨🇳 英译中', desc: '看英文打中文' },
              ].map(opt => (
                <button
                  key={opt.value}
        ...
        <div className="space-y-3">
          <button onClick={handleExport} className="btn-secondary w-full text-left flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs text-gray-400">备份所有单词和练习记录为 JSON 文件</div>
            </div>
          </button>
          ...
          <button onClick={handleImport} className="btn-secondary w-full text-left flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs text-gray-400">从 JSON 备份文件恢复数据</div>
            </div>
          </button>
          ...
          <button onClick={handleReset} className="btn-danger w-full text-left flex items-center gap-2">
            <span>🔄</span>
            <div>
              <div className="font-medium">重置学习进度</div>
              <div className="text-xs text-red-300">清除所有练习记录（单词本保留）</div>
            </div>
          </button>
        </div>
      </div>
